...
(function (window) {
  ...
  class SignalFirePeerConnection extends window.EventEmitter2 {
    constructor (client, remotePeerId, options = {}) {
      ...
      this._peerConnection.onaddstream = (event) => this.emit('stream', event.stream)
      this._peerConnection.ondatachannel = (event) => this._onDataChannel(event.channel)
    }

    addStream (stream) {
      return this._peerConnection.addStream(stream)
    }
    ...
    getDataChannel (label = null) {
      return new Promise((resolve, reject) => {
        if (!this._ready) {
          reject(new Error('client not ready'))
        } else if (label === null) {
          reject(new Error('label can not be null'))
        } else if (!this._dataChannels[label]) {
          reject(new Error('channel not found'))
        } else {
          resolve(this._dataChannels[label])
        }
      })
    }
